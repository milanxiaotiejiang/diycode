package com.seabreeze.log.inner;

import android.text.TextUtils;
import android.util.Log;

import com.seabreeze.log.Print;
import com.seabreeze.log.common.LogConstant;
import com.seabreeze.log.common.LogConvert;
...
public abstract class Tree implements ITree {
    ...
    private StackTraceElement getCurrentStackTrace() {
        ...
        int stackOffset = getStackOffset(trace, Print.class);
        if (stackOffset == -1) {
            return null;
        }
    ...
    private int getStackOffset(StackTraceElement[] trace, Class cla) {
        for (int i = LogConstant.MIN_STACK_OFFSET; i < trace.length; i++) {
            ...
            if (cla.equals(Print.class) && i < trace.length - 1 && trace[i + 1].getClassName()
                    .equals(Print.class.getName())) {
                continue;
            }
